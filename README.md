...
- **🖌️ 紫毫纠错模型**：参考商业输入法的同步纠错思路，在鼠须管模糊音基础上叠加万象语法模型词频预判，用最小代码 + 精简词典实现**零延迟**纠错，覆盖临键错打与近音混淆两类场景；可开关、可调整候选位置与数量。

### 🌟 v2.0.0 重点更新

- **回归苹果原生界面**：v2.0.0 把界面彻底从自绘 HStack 布局搬回 `NavigationSplitView` 原生结构，并在 macOS 27 上验证无系统渲染 bug（消除此前 macOS 27 下的折叠胶囊占位与 detail 顶部半透明暗条）。
- **侧边栏重构**：把命名小耗子（应用图标）、版本号、最重要的「应用并重新部署」按钮全部收进侧边栏，操作路径更短。
- **用户自定义配色增强**：最多可同时启用 **3 套**自定义配色方案，方案列表可容纳更多方案，深色模式配色下拉内也可直接选中自定义方案。
- **外观面板卡顿修复**：根因是字体族同步构建（474 项）阻塞首帧，改为 popover + 懒加载搜索后彻底消除。
- **备份策略调整**：取消「每次应用并重新部署自动备份」，改为纯手动备份，避免 `.bak` 文件堆积占空间。
- **输入方案可单独删除**：非鼠须管自带、且你完全不用的方案（如五笔、繁体、双拼、九笔等）现在可一键移除，保持系统整洁。

![外观与配色](docs/v2.0.0-appearance-colors.png)

![备份与同步](docs/v2.0.0-backup-sync.png)

![方案删除](docs/v2.0.0-schemas-delete.png)

![紫毫纠错模型](docs/v2.0.0-amethyst-correction.png)
